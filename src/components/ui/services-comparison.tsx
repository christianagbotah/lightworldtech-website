...
function CellValue({ value, highlight }: { value: string | boolean; highlight: boolean }) {
  ...
  return <span className={`text-sm font-medium ${highlight ? 'text-emerald-700 dark:text-emerald-300' : 'text-slate-700 dark:text-slate-300'}`}>{value}</span>;
}
...
function MobileServiceCard({ serviceKey, data }: { serviceKey: string; data: ComparisonRow[] }) {
  ...
  return (
    ...
    >
      <Card className={`border-2 overflow-hidden ${info?.popular ? 'border-emerald-400 dark:border-emerald-600' : 'border-slate-200 dark:border-slate-700'}`}>
        {/* Header */}
        <div className={`relative p-5 ${info?.popular ? 'bg-gradient-to-r from-emerald-500 to-emerald-600 dark:from-emerald-600 dark:to-emerald-700' : 'bg-slate-100 dark:bg-slate-800'}`}>
          <div className="flex items-center gap-3">
            <div className={`size-10 rounded-lg flex items-center justify-center ${info?.popular ? 'bg-white/20' : 'bg-white dark:bg-slate-700'}`}>
              <IconComp className={`size-5 ${info?.popular ? 'text-white' : 'text-emerald-600 dark:text-emerald-400'}`} />
            </div>
            <div className="flex-1 min-w-0">
              ...
              <p className={`text-xs mt-0.5 ${info?.popular ? 'text-emerald-100' : 'text-slate-500 dark:text-slate-400'}`}>{info?.desc}</p>
            </div>
          </div>
          {info?.popular && (
                    ...
                    <TooltipProvider>
                      <Tooltip>
                        <TooltipTrigger asChild>
                          <button className="text-slate-400 hover:text-emerald-500 transition-colors shrink-0" aria-label={`Info about ${row.feature}`}>
                      ...
                      {comparisonData.map((row, index) => {
                        ...
                        return (
                          <motion.tr
                            ...
                            className={`group transition-colors ${isDiff ? 'bg-amber-50/40 dark:bg-amber-900/5 hover:bg-amber-50/70 dark:hover:bg-amber-900/10' : 'hover:bg-emerald-50/50 dark:hover:bg-emerald-900/10'}`}
                          >
                            ...
                            <td className={`py-3.5 px-4 border-b border-slate-100 dark:border-slate-800 text-center ${isDiff ? 'bg-emerald-50/30 dark:bg-emerald-900/10' : ''}`}>
