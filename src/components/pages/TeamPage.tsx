        ...
        <div className="container-main">
          <div className="grid gap-4 lg:grid-cols-2">
            {leadership.map((person, index) => (
              ...
              >
                <div className="absolute -right-20 -top-20 size-60 rounded-full bg-amber-400/10 blur-3xl" />
                <div className="relative">
                  <div className="flex items-start justify-between gap-6">
                    <div className="flex size-20 items-center justify-center rounded-[26px] border border-amber-500/15 bg-gradient-to-br from-amber-500/15 to-amber-400/10 text-2xl font-semibold tracking-[-0.04em] text-amber-700 dark:text-amber-300">
                      {person.initials}
                    </div>
                    <span className="font-mono text-[10px] text-slate-300 dark:text-white/15">0{index + 1}</span>
                  </div>
                  <p className="mt-8 text-[10px] font-semibold uppercase tracking-[0.2em] text-amber-600 dark:text-amber-400">{person.role}</p>
      ...
      <section className="section-padding border-y border-slate-200/70 bg-slate-950 text-white dark:border-white/[0.06] dark:bg-[#081119]">
        <div className="container-main">
          <div className="max-w-3xl">
            <p className="text-xs font-semibold uppercase tracking-[0.22em] text-amber-300">{contentText(settings, 'team_model_eyebrow', 'Leadership model')}</p>
          ...
          <div className="relative overflow-hidden rounded-[36px] bg-gradient-to-br from-amber-500 to-amber-700 p-7 text-white sm:p-10 lg:p-12">
            ...
            <div className="relative grid gap-7 lg:grid-cols-[1fr_auto] lg:items-end">
              <div>
                ...
                <p className="mt-4 max-w-2xl text-sm leading-7 text-amber-50/75 sm:text-base">
                  Work with the team on digital products, enterprise systems, AI-enabled workflows, infrastructure, training and advisory.
                </p>
              </div>
              <Link href="/contact" className="inline-flex h-12 items-center justify-center gap-2 rounded-full bg-white px-6 text-sm font-semibold text-amber-800">
