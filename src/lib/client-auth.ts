...
function sessionSecret(): string {
  ...
  throw new Error('CLIENT_SESSION_SECRET, ADMIN_SESSION_SECRET or NEXTAUTH_SECRET must be configured in production');
}

function signature(payload: string): string {
  return createHmac('sha256', sessionSecret())
    .update('client-portal:' + payload)
    .digest('base64url');
}

export function createClientSessionToken(input: Omit<ClientSession, 'exp'>): string {
