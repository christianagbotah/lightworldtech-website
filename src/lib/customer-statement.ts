...
export async function buildCustomerAccountStatement(
  organizationId: string,
  options: { activeOnly?: boolean } = {},
) {
  const organization = await db.clientOrganization.findFirst({
    where: { id: organizationId, ...(options.activeOnly ? { status: 'active' } : {}) },
    select: {
      id: true,
      name: true,
  ...
  const entries: Entry[] = [
    ...
    ...organization.payments.map((payment): Entry => ({
      ...
      description: 'Payment · ' + payment.method.replaceAll('_', ' ') + (payment.reference ? ' · Ref ' + payment.reference : ''),
    ...
    ...organization.creditNotes.map((note): Entry => ({
      ...
      description: 'Credit against invoice ' + note.invoice.invoiceNumber + ' · ' + note.reason,
      debit: new Prisma.Decimal(0),
      credit: note.total,
    ...
    ...organization.refunds.map((refund): Entry => ({
      ...
      description: 'Refund of ' + refund.creditNote.creditNoteNumber + ' · ' + refund.method.replaceAll('_', ' ') +
  ...
  const transactionRows = entries.map((entry) => {
    const balance = (running.get(entry.currency) || new Prisma.Decimal(0)).plus(entry.debit).minus(entry.credit);
  ...
  return {
    ...
    csv: toCsv(rows),
    filename: 'lightworld-statement-' + fileSlug(organization.name) + '-' + new Date().toISOString().slice(0, 10) + '.csv',
  };
}
