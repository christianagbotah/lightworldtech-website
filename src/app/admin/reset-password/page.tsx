...
export default function AdminResetPasswordPage() {
  ...
  const submit = async (event: FormEvent) => {
    ...
    if (!token) {
      setError('This reset link is incomplete. Request a new reset link from the admin sign-in page.');
      return;
    }
  ...
  return (
    <main className="min-h-screen bg-[#050b10] px-4 py-16 text-white">
      <div className="mx-auto flex min-h-[75vh] max-w-lg items-center">
        <Card className="w-full border-white/10 bg-white/[0.04] text-white shadow-2xl">
          <CardContent className="p-7 sm:p-9">
            <div className="mb-7 flex items-center gap-3">
              ...
              <div>
                <h1 className="text-xl font-semibold">Reset administrator password</h1>
                <p className="text-sm text-white/45">Lightworld Technologies CMS</p>
              </div>
            </div>
            ...
            {success ? (
              <div className="space-y-5">
                ...
                <Button asChild className="w-full bg-amber-500 text-slate-950 hover:bg-amber-400">
                  <Link href="/admin">Return to admin sign in</Link>
                </Button>
              </div>
            ) : (
              <form onSubmit={submit} className="space-y-5">
                <p className="text-sm leading-6 text-white/55">
                  Choose a new password with at least 12 characters. Completing the reset invalidates older administrator sessions.
                </p>
                ...
                <div className="text-center">
                  <Link href="/admin" className="text-sm text-white/45 hover:text-amber-300">
