...
export async function POST(
  ...
  if (ticket.status === 'closed') {
    return NextResponse.json({ success: false, error: 'Closed tickets cannot accept new attachments' }, { status: 409 });
  }
  ...
  try {
    ...
    const attachment = await db.$transaction(async (tx) => {
      const created = await tx.clientTicketAttachment.create({
        data: {
          ...
          uploadedByName: context.user.name,
        },
      });
    ...
    console.error('Client support attachment upload failed:', error);
